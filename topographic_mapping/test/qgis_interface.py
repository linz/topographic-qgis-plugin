...
class MockedVectorLayerTools(QgsVectorLayerTools):
    def startEditing(self, layer: QgsVectorLayer):
        layer.startEditing()
        return True

    def stopEditing(self, layer: QgsVectorLayer, allow_cancel=True):
        layer.rollBack()
        return True

    def saveEdits(self, layer: QgsVectorLayer):
        layer.commitChanges()
        return True
    ...
    def addFeature(
        ...
        for idx, value in defaultValues.items():
            feature[idx] = value
        feature.setGeometry(defaultGeometry)
