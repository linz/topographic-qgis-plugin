...
class TestMarkupManager(TopographicTestBase):
    ...
    def test_markup_layers_in_project(self):
        ...
        self.assertIsNone(manager.project_point_markup_layer(project))
        self.assertIsNone(manager.project_line_markup_layer(project))
        self.assertIsNone(manager.project_polygon_markup_layer(project))
        ...
        self.assertIsNone(manager.project_point_markup_layer(project))
        self.assertIsNone(manager.project_line_markup_layer(project))
        self.assertEqual(manager.project_polygon_markup_layer(project), poly)

        line = manager.load_line_markup_layer()
        project.addMapLayer(line)
        self.assertFalse(manager.project_has_all_markup_layers(project))
        self.assertIsNone(manager.project_point_markup_layer(project))
        self.assertEqual(manager.project_line_markup_layer(project), line)
        self.assertEqual(manager.project_polygon_markup_layer(project), poly)

        point = manager.load_point_markup_layer()
        project.addMapLayer(point)
        self.assertTrue(manager.project_has_all_markup_layers(project))
        self.assertEqual(manager.project_point_markup_layer(project), point)
        self.assertEqual(manager.project_line_markup_layer(project), line)
        self.assertEqual(manager.project_polygon_markup_layer(project), poly)
